...
var TopicZhan = require('../proxy').TopicZhan;
  ...
  Topic.getFullTopic(topic_id, ep.done(function (message, topic, tags, author, replies) {
    ...
    topic.save(ep.done(function () {
      ...
      if (!req.session.user) {
		...
		TopicZhan.getTopicZhan(req.session.user._id, topic._id, ep.done(function (doc) {
          topic.in_zhan = doc;
          ep.emit('topic', topic);
        }));
      }
...
exports.put = function (req, res, next) {
  ...
  //describe 是内容截取其中的前140个子的内容
  var describe = content.substr(0,140);
  
  var image='';
  //这边是设置对应的标签内容、
   var tags =req.body.t_tags;
   var topic_tags = [];
   
   var topic_tags = [];
   if (req.body.t_tags !== '') {
        topic_tags = req.body.t_tags.split(',');
      }
  ...
  var uid = req.session.user._id.toString();
   ...
   image ='../../public/user_data/images/'+uid+'/'+filename;
  ...
  if (title === '') {
    ...
    Topic.newAndSave(title, content, describe,image,req.session.user._id, function (err, topic) {
      if (err) {
        return next(err);
      }
...
exports.zhan = function (req, res, next) {
	//这个是收集内容内容的信息
  var topic_id = req.body.topic_id;
  //找到对应的话题
  Topic.getTopic(topic_id, function (err, topic) {
    ...
    TopicZhan.getTopicZhan(req.session.user._id, topic._id, function (err, doc) {
      if (err) {
        return next(err);
      }
      if (doc) {
      ...
      TopicZhan.newAndSave(req.session.user._id, topic._id, function (err) {
        if (err) {
          return next(err);
        }
        res.json({status: 'success'});
      ...
      User.getUserById(req.session.user._id, function (err, user) {
        ...
        user.zhan_topic_count += 1;
        user.save();
      });
	
	  //当前标签的收集认识甲乙
      req.session.user.zhan_topic_zhan += 1;
      topic.zhan_count+= 1;
      topic.save();
    });
  });
...
exports.de_zhan = function (req, res, next) {
  var topic_id = req.body.topic_id;
  Topic.getTopic(topic_id, function (err, topic) {
    if (err) {
      return next(err);
    ...
    TopicZhan.remove(req.session.user._id, topic._id, function (err) {
      if (err) {
        return next(err);
      }
      res.json({status: 'success'});
    ...
    req.session.user.zhan_topic_count -= 1;
  });
};
